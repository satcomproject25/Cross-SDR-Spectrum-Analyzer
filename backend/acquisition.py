...
from .calibration import load_device_calibration, frequency_correction_hz
...
class HackRFAcquisition:
    ...
    def _open_locked(self):
        ...
        try:
            ...
            # Fixed offset + oscillator ppm + measured non-linear residual, all
            # solved from the calibration workbook by backend/cal_workbook.py.
            # Terms absent from calibration.json contribute exactly zero, so a
            # board calibrated with only a fixed offset behaves as it always did.
            axis_offset = frequency_correction_hz(driver_freq, calibration)
    ...
    def run(
        ...
        try:
            ...
            pipeline = AnalyzerPipeline(
                self.config, info.device_name, info.serial_number
            )
